...
proc setMfsRoot(adder: Adderr, r: Root) = 
	adder.mroot = r

# Constructs a node from reader's data, and adds it. Doesn't pin.
proc add(adder: Adder , reader: Reader) ipld.Node = 
	chnk := chunker.fromString(reader, adder.Chunker)
...
proc addFile(adder: Adder, file: File): string =
    ...
    sdata = unixfs.symlinkData(s.target)
	...
	if adder.progress: 
		var rdr = progressReader(file: file, out: out)

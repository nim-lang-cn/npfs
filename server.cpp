      ...
      draining_(false)
{
  ...
  ev_timer_init(&timer_, timeoutcb, 0.,
                static_cast<double>(config.timeout) / (uint64_t)1000000000ULL);
  timer_.data = this;
...
void Handler::on_stream_open(int64_t stream_id)
{
  ...
  (static_cast<bool>(
       ...
       ? void(0)
       : __assert_fail(
             "it == std::end(streams_)", "all.cpp", 41746, __extension__ __PRETTY_FUNCTION__));
...
void Handler::http_begin_request_headers(int64_t stream_id)
{
  ...
  (static_cast<bool>(
       ...
       ? void(0)
       : __assert_fail(
             "it != std::end(streams_)", "all.cpp", 42005, __extension__ __PRETTY_FUNCTION__));
...
void Handler::http_stream_close(int64_t stream_id, uint64_t app_error_code)
{
  ...
  if (ngtcp2_is_bidi_stream(stream_id))
  {
    ...
    (static_cast<bool>(
         ...
         ? void(0)
         : __assert_fail(
               "!ngtcp2_conn_is_local_stream(conn_, stream_id)", "all.cpp", 42159, __extension__ __PRETTY_FUNCTION__));
...
void Handler::write_qlog(const void *data, size_t datalen)
{
  ...
  (static_cast<bool>(
       ...
       ? void(0)
       : __assert_fail(
             "qlog_", "all.cpp", 42371, __extension__ __PRETTY_FUNCTION__));
                  ...
                  uint32_t version)
{
  ...
  std::generate(scid_.data, scid_.data + scid_.datalen, [&dis]() { return dis(randgen); });
  ...
  if (!config.qlog_dir.empty())
  {
    ...
    if (qlog_ == nullptr)
    {
      std::cerr << "Could not open qlog file " << path << ": " << strerror((*__errno_location())) << std::endl;
      return -1;
    }
  ...
  if (config.preferred_ipv4_addr.len || config.preferred_ipv6_addr.len)
  {
    ...
    if (config.preferred_ipv4_addr.len)
    {
      ...
      (static_cast<bool>(
           sizeof(dest) == sizeof(addr.su.in.sin_addr))
           ? void(0)
           : __assert_fail(
                 "sizeof(dest) == sizeof(addr.su.in.sin_addr)", "all.cpp", 42501, __extension__ __PRETTY_FUNCTION__));
    ...
    if (config.preferred_ipv6_addr.len)
    {
      ...
      (static_cast<bool>(
           sizeof(dest) == sizeof(addr.su.in6.sin6_addr))
           ? void(0)
           : __assert_fail(
                 "sizeof(dest) == sizeof(addr.su.in6.sin6_addr)", "all.cpp", 42509, __extension__ __PRETTY_FUNCTION__));
...
void Handler::update_endpoint(const ngtcp2_addr *addr)
{
  ...
  (static_cast<bool>(
       ...
       ? void(0)
       : __assert_fail(
             "endpoint_", "all.cpp", 42556, __extension__ __PRETTY_FUNCTION__));
}
...
void Handler::reset_idle_timer()
{
  ...
  timer_.repeat =
      idle_expiry > now
          ? static_cast<ev_tstamp>(idle_expiry - now) / ((uint64_t)1000000000ULL)
          : 1e-9;
  if (!config.quiet)
  {
...
int Handler::write_streams()
{
  ...
  for (;;)
  {
    ...
    if (nwrite < 0)
    {
      ...
      case NGTCP2_ERR_STREAM_SHUT_WR:
        ...
        (static_cast<bool>(
             ...
             ? void(0)
             : __assert_fail(
                   "ndatalen == -1", "all.cpp", 42728, __extension__ __PRETTY_FUNCTION__));
      ...
      case NGTCP2_ERR_WRITE_MORE:
        ...
        (static_cast<bool>(
             ...
             ? void(0)
             : __assert_fail(
                   "ndatalen > 0", "all.cpp", 42739, __extension__ __PRETTY_FUNCTION__));
        if (auto rv =
      ...
      (static_cast<bool>(
           ...
           ? void(0)
           : __assert_fail(
                 "ndatalen == -1", "all.cpp", 42751, __extension__ __PRETTY_FUNCTION__));
    ...
    (static_cast<bool>(
         ...
         ? void(0)
         : __assert_fail(
               "ndatalen == -1", "all.cpp", 42757, __extension__ __PRETTY_FUNCTION__));
    if (nwrite == 0)
    {
      if (bufpos - buf.data())
      {
...
void Handler::start_draining_period()
{
  ...
  ev_timer_stop(loop_, &rttimer_);
  timer_.repeat =
      static_cast<ev_tstamp>(ngtcp2_conn_get_pto(conn_)) / ((uint64_t)1000000000ULL) * 3;
  ev_timer_again(loop_, &timer_);
  if (!config.quiet)
  {
...
int Handler::send_conn_close()
{
  ...
  (static_cast<bool>(
       conn_closebuf_ && conn_closebuf_->size())
       ? void(0)
       : __assert_fail(
             "conn_closebuf_ && conn_closebuf_->size()", "all.cpp", 42901, __extension__ __PRETTY_FUNCTION__));
...
void Handler::schedule_retransmit()
{
  ...
  auto t = expiry < now ? 1e-9
                        : static_cast<ev_tstamp>(expiry - now) / ((uint64_t)1000000000ULL);
  if (!config.quiet)
  {
    ...
    case NGHTTP3_ERR_STREAM_NOT_FOUND:
      if (ngtcp2_is_bidi_stream(stream_id))
      {
        ...
        (static_cast<bool>(
             ...
             ? void(0)
             : __assert_fail(
                   "!ngtcp2_conn_is_local_stream(conn_, stream_id)", "all.cpp", 43031, __extension__ __PRETTY_FUNCTION__));
        ngtcp2_conn_extend_max_streams_bidi(conn_, 1);
      }
      break;
    default:
...
namespace
{
                  ...
                  int family)
  {
    ...
    {
      std::cerr << "getsockname: " << strerror((*__errno_location())) << std::endl;
      close(fd);
      return -1;
    }
    local_addr.len = len;
...
namespace
{
  ...
  {
    auto fd = socket(addr.su.sa.sa_family,
                     SOCK_DGRAM, 0);
    if (fd == -1)
    {
      std::cerr << "socket: " << strerror((*__errno_location())) << std::endl;
      return -1;
    }
    int val = 1;
    if (addr.su.sa.sa_family ==
            10 &&
        setsockopt(fd,
                   IPPROTO_IPV6,
                   26, &val,
    ...
    {
      std::cerr << "bind: " << strerror((*__errno_location())) << std::endl;
      close(fd);
      return -1;
    }
...
int Server::init(const char *addr, const char *port)
{
  ...
  if (!util::numeric_host(addr,
                          10) &&
      add_endpoint(endpoints_, addr, port,
                   2) == 0)
  {
    ready = true;
  }
...
int Server::on_read(Endpoint &ep)
{
  ...
  for (; pktcnt < 10;)
  {
    ...
    if (nread == -1)
    {
      if (!(
              ...
              (*__errno_location()) ==
                  107))
      {
        std::cerr << "recvmsg: " << strerror((*__errno_location())) << std::endl;
      }
      return 0;
    }
    ++pktcnt;
    ...
    if (handler_it == std::end(handlers_))
    {
      ...
      (static_cast<bool>(
           handler_it != std::end(handlers_))
           ? void(0)
           : __assert_fail(
                 "handler_it != std::end(handlers_)", "all.cpp", 43483, __extension__ __PRETTY_FUNCTION__));
    }
                                     ...
                                     const sockaddr *sa, socklen_t salen)
{
  ...
  static_assert(sv.size() >=
                1 + (0xff000020u - 0xff00001du + 1));
  sv[0] = generate_reserved_version(sa, salen, version);
  size_t svlen = 1;
  for (auto v = 0xff00001du; v <= 0xff000020u; ++v)
  {
    sv[svlen++] = v;
  }
...
namespace
{
                                  ...
                                  const ngtcp2_cid *retry_scid)
  {
    ...
    (static_cast<bool>(
         destlen >= salen + retry_scid->datalen)
         ? void(0)
         : __assert_fail(
               "destlen >= salen + retry_scid->datalen", "all.cpp", 43696, __extension__ __PRETTY_FUNCTION__));
                               ...
                               const sockaddr *sa, socklen_t salen)
{
  ...
  if (!config.quiet)
  {
    std::cerr << "Verifying Retry token from [" << host.data()
              << "]:" << port.data() << std::endl;
    util::hexdump(
        stderr, hd->token.base, hd->token.len);
  }
  ...
  (static_cast<bool>(
       hd->token.base[0] == RETRY_TOKEN_MAGIC)
       ? void(0)
       : __assert_fail(
             "hd->token.base[0] == RETRY_TOKEN_MAGIC", "all.cpp", 43785, __extension__ __PRETTY_FUNCTION__));
  ...
  (static_cast<bool>(
       ciphertextlen >= ngtcp2_crypto_aead_taglen(&token_aead_))
       ? void(0)
       : __assert_fail(
             "ciphertextlen >= ngtcp2_crypto_aead_taglen(&token_aead_)", "all.cpp", 43819, __extension__ __PRETTY_FUNCTION__));
  ...
  uint64_t now = std::chrono::duration_cast<std::chrono::nanoseconds>(
                     std::chrono::system_clock::now().time_since_epoch())
                     .count();
  if (t + 10ULL * ((uint64_t)1000000000ULL) < now)
  {
    if (!config.quiet)
    {
...
namespace
{
  size_t generate_token_aad(uint8_t *dest, size_t destlen, const sockaddr *sa)
  {
    ...
    (static_cast<bool>(
         ...
         ? void(0)
         : __assert_fail(
               "destlen >= addrlen", "all.cpp", 43886, __extension__ __PRETTY_FUNCTION__));
                         ...
                         socklen_t salen)
{
  ...
  (static_cast<bool>(
       hd->token.base[0] == TOKEN_MAGIC)
       ? void(0)
       : __assert_fail(
             "hd->token.base[0] == TOKEN_MAGIC", "all.cpp", 43969, __extension__ __PRETTY_FUNCTION__));
  std::array<uint8_t, 256> aad;
  auto aadlen = generate_token_aad(aad.data(), aad.size(), sa);
  auto rand_data = hd->token.base + hd->token.len - TOKEN_RAND_DATALEN;
  auto ciphertext = hd->token.base + 1;
  ...
  (static_cast<bool>(
       ...
       ? void(0)
       : __assert_fail(
             "ciphertextlen >= ngtcp2_crypto_aead_taglen(&token_aead_)", "all.cpp", 44002, __extension__ __PRETTY_FUNCTION__));
  auto plaintextlen = ciphertextlen - ngtcp2_crypto_aead_taglen(&token_aead_);
  if (plaintextlen != sizeof(uint64_t))
  {
    if (!config.quiet)
  ...
  uint64_t now = std::chrono::duration_cast<std::chrono::nanoseconds>(
                     std::chrono::system_clock::now().time_since_epoch())
                     .count();
  if (t + 3600ULL * ((uint64_t)1000000000ULL) < now)
  {
    if (!config.quiet)
    {
      std::cerr << "Token has been expired" << std::endl;
                        ...
                        size_t gso_size)
{
  ...
  int len;
   char *pbuf = NULL;
  FILE* fp = fopen("/mnt/d/videos/balanced2.png", "r");
  if( fp == NULL ) 
   {
      perror ("打开文件错误");
      return(-1);
  ...
  } while (nwrite == -1 && (*__errno_location()) == 4);
  if (nwrite == -1)
  {
    std::cerr << "sendmsg: " << strerror((*__errno_location())) << std::endl;
    return NETWORK_ERR_OK;
  }
  if (!config.quiet)
  {
...
namespace
{
  ...
  {
    auto h = static_cast<Handler *>((SSL_get_ex_data(ssl, 0)));
    const uint8_t *alpn;
    size_t alpnlen;
...
namespace
{
  SSL_CTX *create_ssl_ctx(const char *private_key_file, const char *cert_file)
  {
    ...
    constexpr auto ssl_opts = ((0x80000000U | 0x00000800U | 0x00000004U | 0x00000010U | 0x00000040U) & ~0x00000800U) |
                              0x0 |
                              0x00400000U |
                              0x01000000U;
    ...
    SSL_CTX_ctrl((ssl_ctx), 33, (0x00000010U),__null);
    ...
    {
      SSL_CTX_set_verify(ssl_ctx, 0x01 | 0x04 | 0x02, verify_cb);
    }
...
namespace
{
  void config_set_default(Config &config)
  {
    ...
    config.timeout = 30 * ((uint64_t)1000000000ULL);
    {
      ...
      (static_cast<bool>(
           ...
           ? void(0)
           : __assert_fail(
                 "path", "all.cpp", 44386, __extension__ __PRETTY_FUNCTION__));
      config.htdocs = path;
      free(path);
    }
...
int main(int argc, char **argv)
{
  ...
  for (;;)
  {
    ...
    constexpr static option long_opts[] = {
        ...
        {"htdocs", 1, nullptr, 'd'},
        {"quiet", 0, nullptr, 'q'},
        {"show-secret", 0, nullptr, 's'},
        {"validate-addr", 0, nullptr, 'V'},
        {"ciphers", 1, &flag, 1},
        {"groups", 1, &flag, 2},
        {"timeout", 1, &flag, 3},
        {"preferred-ipv4-addr", 1, &flag, 4},
        {"preferred-ipv6-addr", 1, &flag, 5},
        {"mime-types-file", 1, &flag, 6},
        {"early-response", 0, &flag, 7},
        {"verify-client", 0, &flag, 8},
        {"qlog-dir", 1, &flag, 9},
        {"no-quic-dump", 0, &flag, 10},
        {"no-http-dump", 0, &flag, 11},
        {"max-data", 1, &flag, 12},
        {"max-stream-data-bidi-local", 1, &flag, 13},
        {"max-stream-data-bidi-remote", 1, &flag, 14},
        {"max-stream-data-uni", 1, &flag, 15},
        {"max-streams-bidi", 1, &flag, 16},
        {"max-streams-uni", 1, &flag, 17},
        {"max-dyn-length", 1, &flag, 18},
        {"cc", 1, &flag, 19},
        {"initial-rtt", 1, &flag, 20},
        {"max-udp-payload-size", 1, &flag, 21},
        {"send-trailers", 0, &flag, 22},
        {"max-window", 1, &flag, 23},
        {"max-stream-window", 1, &flag, 24},
        {nullptr, 0, nullptr, 0}};
    auto optidx = 0;
    ...
    case 0:
      ...
      case 3:
        ...
        break;
      case 4:
        if (parse_host_port(config.preferred_ipv4_addr, 2, optarg, optarg + strlen(optarg)) != 0)
        {
        ...
        break;
      case 5:
        if (parse_host_port(config.preferred_ipv6_addr, 10, optarg, optarg + strlen(optarg)) != 0)
        {
          std::cerr << "preferred-ipv6-addr: could not use '" << optarg << "'" << std::endl;
          exit(1);
        }
        break;
      case 6:
  ...
  auto ev_loop_d = defer(ev_loop_destroy, ev_default_loop(0));
  ...
  Server s(ev_default_loop(0), ssl_ctx);
